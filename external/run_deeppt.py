#!/usr/bin/env python
"""Stage 5 driver -- DeepPT (deeppt_833_raw, LAST-epoch MLP), her2st held-out + Visium.

Needs deeppt_resave_last.py to have run first (it writes <P>_mlp_last.pt).
...
Per fold P:
  ...
  MLP       <P>_mlp_last.pt  (Linear -> Dropout -> Linear, eval mode)
  truth     the port's 02_build_targets rule: CP10K over the FULL count row (all detected
            genes), panel subset, log10(x + 1)
  round-trip  P's her2st sections from the cached features_raw vs the resave run's stored
            last-epoch predictions (same weights: expect an exact match)
...
def main():
    ...
    ap.add_argument("--run", default="/workspace/deeppt/results/deeppt_833_raw_lastckpt")
    ap.add_argument("--ae_run", default="/workspace/deeppt/results/deeppt_833_raw_lastckpt",
                    help="where <P>_ae.pt lives (the resave run refits and saves it too)")
    ...
    for fd in folds:
        ...
        ae.load_state_dict(torch.load(os.path.join(a.ae_run, "ckpt", f"{P}_ae.pt"), map_location=dev))
        mlp = Predictor(512, 512, len(panel), 0.2).to(dev)
        mlp.load_state_dict(torch.load(os.path.join(a.run, "ckpt", f"{P}_mlp_last.pt"), map_location=dev))
        ae.eval(); mlp.eval()
        ...
        extra = dict(ae=os.path.join(a.ae_run, "ckpt", f"{P}_ae.pt"),
                     mlp=os.path.join(a.run, "ckpt", f"{P}_mlp_last.pt"),
        ...
        if not a.skip_roundtrip:
            ...
            last = int(hist.epoch.iloc[-1])
            st = np.load(os.path.join(a.run, "preds", P, f"{P}_{last}.npz"), allow_pickle=True)
            for sec in secs_of[P]:
                ...
                rt.append(dict(model="deeppt", fold=P, section=sec, last_epoch=last,
