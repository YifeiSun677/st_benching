...
Everything printed here should say OK before you launch a real fold.
"""
...
def main() -> None:
    ...
    if missing_cache:
        sys.exit("run scripts/01_build_cache.py first")

    print("\n=== target arithmetic ===")
    meta = her2st.read_meta(names[0])
    X = her2st.expression(meta, panel)
    check("expression shape", X.shape == (len(meta), 833), str(X.shape))
    check("target is log10(CP10K+1)", X.min() >= 0 and X.max() < 5,
          f"min {X.min():.3f} max {X.max():.3f}")
    try:
        import scprep
        ref = scprep.transform.log(scprep.normalize.library_size_normalize(
            np.array([[meta[g].values if g in meta.columns else np.zeros(len(meta))
                       for g in panel]], dtype=float)[0].T))
    ...
    for cv in ("patient", "section"):
        f = her2st.folds(cv)
        print(f"       cv={cv:8s} {len(f):2d} folds, "
              f"fold0 holds out {f[0]['test']}")
    ...
    check("output shape", tuple(y.shape) == (1, patches.shape[0], 833), str(tuple(y.shape)))
